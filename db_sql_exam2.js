...
//11월9일 함수명 수정 및 response status 400 -> 200으로 변경 
//기관정보 요청 함수 
module.exports.institution_request = (req,res)=>{
    ...
    pool.getConnection().then((conn)=>{
        conn.query(`select * from institution where id = ${id}`).then((data)=>{
            ...
            console.log(err)
            res.status(200).send({'key':err.code}); //에러정보 전송 
        })
...
module.exports.institution_insert = (req,res)=>{
...
//기관정보수정
module.exports.institution_modify = (req,res)=>{
   ...
   //이메일 정규식
    ...
    if(!e_mail_check(e_mail))
    {
        ...
        res.status(200).send({'key':1}) //이메일양식에러
    ...
    if(!phone_check(phone_number))
    {
        ...
        res.status(200).send({'key':3}) //담당자번호에러
    }
        ...
        pool.getConnection().then((conn)=>{
            ...
            console.log(err.code);
            res.status(200).send({'key':err.code}); //에러코드전송
            })
...
module.exports.institution_delete = (req,res)=>{ //기관삭제
    ...
    pool.getConnection().then((conn)=>{
        conn.query(`delete from institution where institution_id = ${id}`).then((data)=>{
            ...
            console.log(err.code);
            res.status(200).send({'key':err.code}); //에러원인 전송
        })
        conn.release();
    }).catch((err)=>{
        console.log(err.code);
        res.status(200).send({'key':err.code});
    })

}


module.exports.manager_request = (req,res)=>{ //담당자 조회
...
module.exports.manager_insert = (req,res)=>{
...
module.exports.manager_modify = (req,res)=>{
    let manager_id = req.body.manager_id; //담당자 아이디
    let name = req.body.name; //담당자이름
    let phone_number= req.body.phone_number; //휴대폰번호
    let e_mail = req.body.e_mail; //이메일정보
