...
module.exports.sql_insert_naver= (req,res)=>
{ //네이버 로그인 계정 db에 저장
    let email = req.body.email;
    let social = 'naver';
    pool.getConnection().then((conn)=>
    {
        conn.query(`select * from user_social where email_address ='${email}'`).then((data)=>
        { //해당 이메일이 Db에 저장되어있는지 확인
...
module.exports.sql_alarm_count =(req,res)=>
{ //일별 월별 알람 갯수 전송
    ...
    pool.getConnection().then((conn)=>
    {
        ...
        conn.query(`select count(*) from alarm where date_format(alarm_time,'${lite}')='${date}'`).then((data)=>
        { //일별 월별 알람 데이터 갯수 요청쿼리
